...
|------------|---------|-------------|
| **Implementor** | Writes code, commits, pushes. Follows task note exactly. | All code changes — features, bug fixes, refactors |
| **Verifier** | Evidence-driven review against acceptance criteria. | After each wave completes, and after all waves |
| **UI Designer** | Builds accessible, design-system-consistent interfaces. | Tasks that are primarily UI/frontend work |
| **PR Reviewer** | Reviews PR diffs for bugs, security, correctness. | After implementation, before requesting human review |
| **PR Shepherd** | Shepherds a PR to merge-ready (CI green, comments resolved). | After PR is created and needs to reach green |

**Wave composition rules:**
- Tasks in the same wave run in parallel — they MUST NOT touch the same files
- If two tasks might overlap in files, put them in separate waves
- Prefer 2-4 tasks per wave. One mega-task per wave defeats parallelism; 8+ tasks per wave risks conflicts
- UI tasks can often run in parallel with backend tasks (different file trees)
